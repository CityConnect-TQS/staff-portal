...
export function ModalCreateBus({
  company,
  edit,
  bus,
}: {
  ...
  const { Field, handleSubmit } = useForm<BusCreate>({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (edit && bus) {
        const busUpdated = await updateBus(bus.id, busNew, user.token).catch(
          (error) => {
        ...
        const busCreated = await createBus(busNew, user.token).catch(
          (error) => {
            console.error("Error:", error);
            return;
          }
    ...
    <>
      <div className="flex flex-wrap gap-3">
        {edit ? (
          ...
          <Button
            color="primary"
            variant={company ? "flat" : "solid"}
            onClick={onOpen}
            size="sm"
            id={company ? `${company}AddNewBtn` : "addNewBtn"}
          ...
          {(onClose) => (
            <>
              <ModalHeader
                ...
                id="headerModalCreateBus"
              >
                {edit ? "Edit bus" : "Create a new bus"}
              </ModalHeader>
              <ModalBody>
                  ...
                  >
                    {({ state, handleChange, handleBlur }) => (
                      <Input
                        ...
                        isRequired
                        defaultValue={
                          state.value ? state.value.toString() : "0"
                        }
                        onChange={(e) =>
